...
#TRIP ADVISOR CLASS
require_once "adminLoginModel.php";
...
if (isset($_POST["admin_login"]) && $_POST["admin_login"] == "true") {
    ...
    if ($_POST["email"] == "") {
        ...
        $adminLoginRs = $adminModel->adminLogin($_POST["email"], $_POST["pass"]);

        if ($adminLoginRs->num_rows == 1) {
            $oneRow = $adminLoginRs->fetch_assoc();
            $_SESSION["ADMIN"] = $oneRow;
            setcookie(
                'uuid',
                '1',
                time() + (24 * 60 * 60)
            ...
            header("Location: ../index.php");
            exit;
        } else if ($adminLoginRs->num_rows == 0) {
...
<head>
    ...
    <title>Encora | Admin Login</title>
                                        ...
                                        <div class="col-12 mt-3">
                                            <div class="row mx-5 p-2">
                                                <form action="" method="POST">
                                                    ...
                                                    <button name="admin_login" value="true" type="submit" class="mb-5 mt-3 btn btn-warning">Login</button>
                                                </form>
                                            </div>
                                        </div>
                                    </div>
